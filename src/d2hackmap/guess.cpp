...
7442,
	...
	int fireMin=D2GetUnitStat(PLAYER, 48, 0);
	int fireMax=D2GetUnitStat(PLAYER, 49, 0);
	int lv=0;
	if (fireMin<504) lv=0;
...
*/
	...
	int level=D2GetUnitStat(PLAYER, STAT_LEVEL, 0);
	...
	for (UnitAny *pUnit = D2GetFirstItemInInv(PLAYER->pInventory);pUnit;pUnit = D2GetNextItemInInv(pUnit)) {
		...
		switch (dwQuality) {
			...
			case ITEM_QUALITY_UNIQUE: {
				int fileindex = D2GetItemFileIndex(pUnit);
		...
		add_eng+=D2GetUnitStat(pUnit, STAT_ENERGY, 0);
		add_mana+=D2GetUnitStat(pUnit, STAT_MAXMANA, 0)>>8;
		add_mana_lv+=D2GetUnitStat(pUnit, 217, 0);
		add_eng_lv+=D2GetUnitStat(pUnit, 223, 0);
		add_mana_percent+=D2GetUnitStat(pUnit, 77, 0);
		add_vit+=D2GetUnitStat(pUnit, STAT_VITALITY, 0);
		add_hp+=D2GetUnitStat(pUnit, STAT_MAXHP, 0)>>8;
		add_hp_lv+=D2GetUnitStat(pUnit, 216, 0);
		add_vit_lv+=D2GetUnitStat(pUnit, 222, 0);
		add_hp_percent+=D2GetUnitStat(pUnit, 76, 0);
	}
	...
	int mana_actual=D2GetUnitStat(PLAYER, STAT_MAXMANA, 0)>>8;
	int hp_actual=D2GetUnitStat(PLAYER, STAT_MAXHP, 0)>>8;
	if (debug&&logfp) {
