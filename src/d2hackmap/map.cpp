...
BYTE nNeutralPlayerColor=0x62;
BYTE nHostilePlayerColor=0x5b;
...
void automap_fixValues() {
	for (int i=1;i<140;i++) if (waypointMap[i]) addMapTarget(i,CELLNO_WAYPOINT,1,L"Waypoint",L"小站");
}
...
void AutoMapNewLevel() {
	...
	updateTargetName();
	if (dwAutoRevealMode) {
		if (dwAutoRevealMode>=3&&!afRevealedActs[ACTNO]
			||dwAutoRevealMode<3&&pCurMapLevel->revealed<dwAutoRevealMode) {
			int autoReveal=dwPlayerLevel>=dwAutoRevealCLevel;
			if (!autoReveal) {
				static int qids[5]={6,14,22,26,40};
				if (QUESTDATA[0][qids[ACTNO]]&1) autoReveal=1;
			}
...
int PrevMapTarget() {
	...
	for (MinimapLevelTarget *pTarget=pCurMapLevel->targets;pTarget;pTarget=pTarget->next) {
		if (pTarget->next==pCurMapLevel->curTarget) {
...
//ecx = ptr new node
		...
		popad
//original code
		mov edx,d2client_vAutomapNodeListBaseAddr
		mov edx,[4*eax+edx]
		ret
	}
}
void DrawAutomapNode(CellFile *pCellfile, int xpos, int ypos, char color) {
	...
	if ((BYTE)color!=pCellfile->mylastcol) pCellfile->mytabno=pCellfile->mytabno?0:1;
	pCellfile->mylastcol=coltab[pCellfile->mytabno][255]=(BYTE)color;
	d2gfx_DrawCellFile(&ct, xpos, ypos, -1, 5, coltab[pCellfile->mytabno]);
}
...
void MinimapPatch() {
	if (dwRevealMapMs&&dwCurMs>dwRevealMapMs) {
		...
		if (dwAutoRevealMode>=3) RevealCurAct();
			...
			DrlgLevel *pDrlgLevel = PLAYER->pMonPath->pAreaRectData->pAreaRectInfo->pDrlgLevel;
			if (pDrlgLevel) RevealAutomapLevel(pDrlgLevel,dwAutoRevealMode);
		}
	}
