...
static void InitValues(){
	...
	winmsg_addConfigVars();dangerous_addConfigVars();
	LOG("%d config vars\n",configVarCount);
...
int ConvertToWString(wchar_t *dst,char *src,int size) {
	return MultiByteToWideChar(CP_ACP, 0, src, -1, dst, size);
}
...
static int changeLogPath(char *path) {
	...
	if (f) {
		if (logfp) {fprintf(logfp,"Log Changed to %s\n",buf);fclose(logfp);}
		logfp=f;
