...
int d2s_generate(FILE *fp, UnitAny *pUnit,UnitAny *m,u16 *quest,char *wp,UnitAny *ironGolem) {
	...
	if (pUnit&&pUnit->dwUnitType==0) {
		...
		f.header.charLevel=D2GetUnitStat(pUnit, STAT_LEVEL, 0);
		f.header.charStatus=EXPANSION?0x20:0;
		for (int i=0;i<16;i++) {
			int hotkey=p_D2SkillHotKeys[i];
			int left=p_D2SkillHotKeyLeft[i];
		...
		if (m) {
			if (*p_D2MercData16 != 0xFFFF) f.header.mercDead=1;
		...
		if (m) f.header.mercExp=D2GetUnitStat(m, STAT_EXP, 0);
		...
		for (UnitAny *pItem = D2GetFirstItemInInv(pUnit->pInventory);pItem;pItem = D2GetNextItemInInv(pItem)) {
	...
	short *pItemCount=(short *)&buf[size];*pItemCount=0;size+=2;
	if (m) {
		for (UnitAny *pItem = D2GetFirstItemInInv(m->pInventory);pItem;pItem = D2GetNextItemInInv(pItem)) {
			if (size>bufsize-8192) {bufsize*=2;buf=(char *)realloc(buf,bufsize);if (!buf) return -1;}
			int written=encodeItem(pItem,buf+size,bufsize-size);
			size+=written;
			pItemCount[0]++;
	...
	UnitAny *ironGolemItem = ironGolem?D2GetFirstItemInInv(ironGolem->pInventory):NULL;
	if (ironGolemItem) {
		buf[size++]=1;
