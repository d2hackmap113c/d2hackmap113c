...
int init_d2_ptrs(int server) {
  ...
  addr_d2cmp=get_dll_addr("D2CMP.DLL"); if (!addr_d2cmp) return 0;
	if (server) addr_bnclient=(HMODULE)bnclient_ADDR;
  ...
  addr_d2game=get_dll_addr("D2GAME.DLL"); if (!addr_d2game) return 0;
	if (server) {
		addr_d2launch=(HMODULE)d2launch_ADDR;
		addr_d2gdi=(HMODULE)d2gdi_ADDR;
	} else {
  ...
  addr_d2sound=get_dll_addr("D2SOUND.DLL");if (!addr_d2sound) return 0;
	...
	if (server) {
		addr_d2multi=(HMODULE)d2multi_ADDR;
		addr_d2mcpclient=(HMODULE)d2mcpclient_ADDR;
	} else {
