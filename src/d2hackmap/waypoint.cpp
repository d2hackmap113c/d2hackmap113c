...
int takeWaypointToTown(int maxdis) {
	UnitAny *waypoint=findWaypoint(6);
	if (!waypoint) return 0;
	takeWaypointToLevel(waypoint,actlvls[ACTNO]);
	return 1;
}
...
static int __fastcall recvWaypointMenu(char *packet,int len) {
	...
	if (1<=dwTakeWaypointToLevel&&dwTakeWaypointToLevel<140) {
		int bit=waypointMap[dwTakeWaypointToLevel];
		if (dwTakeWaypointToLevel>1&&bit==0) {gameMessage("No waypoint to level %d",dwTakeWaypointToLevel);return 0;}
