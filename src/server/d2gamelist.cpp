...
void D2GSDeleteAllCharInGame(D2GAMEINFO *lpGameInfo)
{
	...
	while(ph)
	{
		...
		if (ph->CharLockStatus) {
			...
			D2GSUnlockChar((LPCSTR)ph->AcctName, (LPCSTR)ph->CharName);
		}
		D2GSEventLog("D2GSDeleteAllCharInGame",
	...
	WORD wGameId)
{
	...
	strncpy((char *)lpGameInfo->GameName, game_name, MAX_GAMENAME_LEN-1);
	strncpy((char *)lpGameInfo->game_pass, game_pass, sizeof(lpGameInfo->game_pass));
	strncpy((char *)lpGameInfo->game_desc, game_desc, sizeof(lpGameInfo->game_desc));
	strncpy((char *)lpGameInfo->creator_acct_name, creator_acct_name, sizeof(lpGameInfo->creator_acct_name));
	strncpy((char *)lpGameInfo->creator_char_name, creator_char_name, sizeof(lpGameInfo->creator_char_name));
	strncpy((char *)lpGameInfo->creator_ip, creator_ip, sizeof(lpGameInfo->creator_ip));
			...
			UCHAR *CharName, UCHAR* RealmIPName, DWORD CharLevel, WORD CharClass, WORD EnterGame)
{
	...
	strncpy((char *)lpCharInfo->AcctName, (char *)AcctName, MAX_ACCTNAME_LEN-1);
	strncpy((char *)lpCharInfo->CharName, (char *)CharName, MAX_CHARNAME_LEN-1);
	strncpy((char *)lpCharInfo->RealmIPName, (char *)RealmIPName, MAX_REALMIPNAME_LEN - 1);
...
int D2GSInsertGetDataRequest(UCHAR *AcctName, UCHAR *CharName, DWORD dwClientId, DWORD dwSeqno)
{
	...
	strncpy((char *)lpGetDataReq->AcctName, (char *)AcctName, MAX_ACCTNAME_LEN-1);
	strncpy((char *)lpGetDataReq->CharName, (char *)CharName, MAX_CHARNAME_LEN-1);
...
D2GAMEINFO *D2GSFindGameInfoByGameName(UCHAR *GameName)
{
	...
	while(lpGame)
	{
		if (!strcmp((char *)lpGame->GameName, (char *)GameName)) break;
		lpGame = lpGame->next;
	}
	LeaveCriticalSection(&csGameList);
	return lpGame;
...
D2CHARINFO *D2GSFindCharInGameByCharName(D2GAMEINFO *lpGame, UCHAR *CharName)
{
	...
	while(lpChar)
	{
		if (!strcmp((char *)lpChar->CharName, (char *)CharName)) break;
		lpChar = lpChar->next;
	}
	LeaveCriticalSection(&csGameList);
	return lpChar;
...
void D2GSGetDataRequestTimerRoutine(void)
{
	...
	while(lpGetDataReq)
	{
		...
		if (lpTimeOutReq) {
			...
			lpBufferTemp = (MOTDCLIENT *)malloc(sizeof(MOTDCLIENT));
			if (!lpBufferTemp)
			{
			...
			strncpy((char *)AcctName, (char *)lpTimeOutReq->AcctName, MAX_ACCTNAME_LEN-1);
			AcctName[MAX_ACCTNAME_LEN-1] = '\0';
			strncpy((char *)CharName, (char *)lpTimeOutReq->CharName, MAX_CHARNAME_LEN-1);
...
void FormatTimeString(long t, char *buf, int len, int format)
{
	struct tm		*tm;
	time_t			now;
	...
	if (format == 0)
	{
		_snprintf((char *)buf, len - 1, "%02d:%02d:%02d", tm->tm_hour, tm->tm_min, tm->tm_sec);
	}
	else if (format == 1)
	{
		_snprintf((char *)buf, len - 1, "%04d-%02d-%02d %02d:%02d:%02d",
...
void D2GSShowGameList(unsigned int ns)
{
	...
	gamecount = charcount = 0;
	sprintf((char *)buf, "+-No.--GameName---------GamePass---------ID----GameVer--Type--Difficulty--Ladder-----Users-CreateTime-Dis-+\r\n");
	SENDSTR(ns, (char *)buf);
	...
	while(lpGame)
	{
		FormatTimeString(lpGame->CreateTime, (char *)timestr, sizeof(timestr), 0);
		sprintf((char *)buf, "| %03d  %-15s  %-15s  %-4u  %-7s  %-4s  %-11s %-10s %-5u %-10s %-3s |\r\n",
			gamecount + 1,
			lpGame->GameName,
			lpGame->game_pass,
			lpGame->GameId,
...
int chat_message_announce_char(DWORD dwMsgType, const char *CharName, const char *msg)
{
	...
	EnterCriticalSection(&csGameList);
	lpChar = (D2CHARINFO *)charlist_getdata((const unsigned char *)CharName, CHARLIST_GET_CHARINFO);
	if (!lpChar) {
		LeaveCriticalSection(&csGameList);
		return -1;
	}
...
int D2GSSendMOTD(void)
{
	...
	strcpy((char *)motd_str, (char *)d2gsconf.motd);
	string_color1(motd_str);
	...
	list_for_each_safe(p, ptemp, &list_motd)
	{
		...
		if (motd_str[0]) chat_message_announce_char2(CHAT_MESSAGE_TYPE_SYS_MESSAGE, pmotd->ClientId, motd_str);
