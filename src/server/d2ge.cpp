...
int D2GEStartup(void)
{
	...
	//printf("prepare to start, press to continue...\n");gets(buf);
...
int D2GEThreadInit(void)
{
	...
	gD2GSInfo.fpErrorHandle			= (FARPROC)D2GSErrorHandle;
 ...
 * Return: TRUE(success) or FALSE(failed)
 *********************************************************************/
LPD2GSINTERFACE (*QueryInterface1)()=NULL;
static BOOL D2GSGetInterface(void)
{
	LPD2GSINTERFACE		lpD2GSInterface;

	/*if (!QueryInterface1) {
		HINSTANCE storm = LoadLibrary("d:\\game\\mpq\\storm.dll");
		HINSTANCE d2server = LoadLibrary("d:\\game\\mpq\\d2server.dll");
		QueryInterface1=(LPD2GSINTERFACE (*)())GetProcAddress(d2server,"QueryInterface");
...
int d2geStart(LPD2GSINFO lpD2GSInfo) {
	printf("---------- D2GSStart --------\n");
	int r=D2GSStart(&gD2GSInfo);
	printf("---------- D2GSStart end %d --------\n",r);
	return r;
}
/*********************************************************************
...
DWORD WINAPI D2GEThread(LPVOID lpParameter)
{
	...
	hObjects[1] = CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)d2geStart, &gD2GSInfo, 0, &dwThreadId);
	if (!hObjects[1]) {
...
void D2GEReloadConfig(void)
{
	...
	_snprintf(buffer, 259, "%s\\%s", PathName, d2gsconf.serverconffile);
	buffer[259] = 0;
	D2GSLoadConfig(buffer);
	ret = D2GSInitConfig();
	if (ret->bHasMsg)
	{
		_snprintf(gWorldEventMessage, 255, "\xFF" "c9The World Event enables on this server. The key item is \"%s\"." "\xFF" "c1", (UCHAR*)(ret + 1));
	}
